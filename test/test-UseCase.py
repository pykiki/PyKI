...
if __name__ == '__main__':
    ...
    # force serverkey to have a size of 1024, and not to be protected by passphrase
    serverkey = pki.create_key(keysize = 1024, name = commoname)
    if serverkey['error'] :
        print("ERROR: Unable to generate server key "+commoname+" properly, aborting...")
        exit(1)
    elif mainVerbosity:
    ...
    if servercert['error'] :
        print("ERROR: Unable to generate server certificate "+commoname+" --> "+servercert['message']+", aborting...")
        exit(1)
    elif mainVerbosity:
    ...
    serverkey = pki.create_key(keysize = 1024, name = commoname)
    if serverkey['error'] :
        print("ERROR: Unable to generate server key "+commoname+" --> "+serverkey['message']+", aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Key "+commoname+" done.")

    if mainVerbosity:
        print("INFO: Generating server certificate which has expired a year ago "+commoname)
    ...
    if expiredcert['error'] :
        print("ERROR: Unable to generate server certificate "+commoname+" --> "+expiredcert['message']+" aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Certificate "+commoname+" done.")
    ...
    if mainVerbosity:
        print("INFO: Generating server certificate which will be valid in 10 years for 13 years "+commoname)
    expiredcert = pki.create_cert(
                              country = 'BE', state = 'Antwerp', city = 'Mechelen',
                              org = 'In Serf we trust, Inc.', ou = 'Test Suite Server',
                              email = 'serfserver@example.com',
    ...
    if mainVerbosity:
        print("INFO: Generating server certificate which will be valid for 13 years with no CommonName "+commoname)
    ...
    if san_nocncert['error'] :
        print("ERROR: Unable to generate server certificate "+commoname+" --> "+san_nocncert['message']+", aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Certificate "+commoname+" done.")
    ...
    clientkey = pki.create_key(passphrase=cltkeyPass, name=commoname, usage='clientAuth')
    if clientkey['error'] :
        print("ERROR: Unable to generate client key "+cltKeyfile+" --> "+clientkey['message']+" aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Key "+commoname+" done.")
    ...
    if key['error'] :
        print("ERROR: Unable to generate client key "+commoname+" --> "+key['message']+" aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Key "+commoname+" done.")
    ...
    crl = pki.revoke_cert(certname = commoname, next_crl_days = 183, reason = "cessationOfOperation")
    if crl['error']:
        print(crl['message'])
    elif mainVerbosity:
        print(crl['message'])
    ...
    if mainVerbosity:
        print("INFO: Revoking certificate "+commoname+"...")
    crl = pki.revoke_cert(certname = commoname, next_crl_days = 183, reason = "keyCompromise")
    if crl['error']:
        print(crl['message'])
    elif mainVerbosity:
        print(crl['message'])
    ...
    # CSR signin'
    commoname = "test_newcsr"
    csrfile = '/opt/PyKI_data/CERTS/requests/'+commoname+'/'+commoname+'.csr'
    if mainVerbosity:
        print("INFO: Signing Certificate Request "+csrfile+" for 90 days of validity...")
    signRes = pki.sign_csr(csr = csrfile, KeyUsage = "clientAuth", days_valid = 90, encryption = "SHA1")
    if signRes['error'] :
        print("ERROR: Unable to generate certificate for csr "+csrfile+" --> "+signRes['message']+" aborting...")
        exit(1)
    elif mainVerbosity:
        print("INFO: Certificate "+commoname+" done.")
    ...
    reschk = pki.check_cer_vs_key(cert = certFromCsr, key = csrkey, keypass = csrPassphrase)
    if reschk['error']:
    ...
    extractres = pki.extract_pkcs12(pkcs12file = pkcsfile, pkcs12pwd = pkcspass, destdir = dstdata)
    if extractres['error']:
    ...
    unprotectres = pki.unprotect_key(keyname = commoname, privKeypass = csrPassphrase)
