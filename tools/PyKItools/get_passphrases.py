...
def argCommandline(argv):
    ...
    """
    parser = argparse.ArgumentParser(description='Retrieve passphrase for a specific certificate name. If not specified, this will return all passphrases')
    parser.add_argument("-a", "--all", action="store_true", dest="all", help=u"Show all passphrases", required=False)
    parser.add_argument("-n", "--cn", action="store", dest="cn", type=str, help=u"Certificate common name", metavar='Common Name', required=False, default=False)
...
if __name__ == '__main__':
    ...
    if not passphrases['error']:
        if args['all']:
            ...
            database_certname = passphrases['message'][pki.cleanStr(args['cn'])]
